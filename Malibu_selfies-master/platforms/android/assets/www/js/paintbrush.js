	...
	function processFilters(filterType, img, params, toFilter, current, buffer, c) {
		...
		if (img && c) {
			...
			if (pixels) {
				...
				//	alert(dest.r+"-"+dest.g+"-"+dest.b+"-"+params.tintOpacity);
				}
...
function getFilterParameters(ref) {
	...
	var params = {
		...
		"tintColor"			:	"#FFFFFF",	// any hex color
	...
	params['tintColor'] = ref.getAttribute("data-pb-tint-colour");
...
//alert(buffer)
	...
	placeReferenceImage(ref, buffer.toDataURL("image/jpeg"), img);
	//console.log("STAsh..."+buffer.toDataURL("image/jpeg"));
	$('#desc2').text("");
...
//	 $("#contentimg").attr("src", buffer.toDataURL("image/jpeg"));
	//$('#desc2').text((buffer.toDataURL("image/jpeg")));
	// and finally, add a class that references the stashed original image for later use
	//alert("cnd>"+document.getElementById('chkuploadimg'));
	console.log("dfhsf>>"+document.getElementById('chkuploadimg'));
